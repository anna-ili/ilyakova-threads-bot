...
- Исходный драфт — опорная версия. Текущий вариант мог стать хуже после прошлой
  правки: не наследуй из него новые абзацы и объяснения автоматически.
...
Исходный драфт до всех итераций:
"""
${originalEn}
"""
...
метафоры, выводы, нейросетевые связки и литературные украшения. Если текущий
вариант раздулся после прошлых итераций, вернись к исходному драфту и исправь
его. Финальная версия не должна быть длиннее исходного драфта. Сохрани живую
разговорную неровность.
...
`.trim();
  ...
  const maxLength = Math.max(Math.round(originalEn.length * 1.15), originalEn.length + 40);
